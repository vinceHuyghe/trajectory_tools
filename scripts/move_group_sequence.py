...
class BaseCmd(_AbstractCmd):
    ...
    def __init__(
        ...
        if not isinstance(
            ...
            (
             ...
             raise ValueError("Goal must be a Pose, PoseStamped or tuple of joint positions")
    ...
    def _cmd_to_request(self, move_group: MoveGroupCommander) -> MotionPlanRequest:
        ...
        req.max_acceleration_scaling_factor = self._acc_scale * self._calc_acc_scale(1.0)
    ...
    def _joint_values_to_constraint(self, joint_names=()):
        if not isinstance(
            ...
            raise TypeError(f"{self._goal} is not convertible into joint values.")
        ...
        if len(joint_names) != len(joint_values):
            raise IndexError(
                "Given joint goal does not match the active joints " + str(joint_names) + "."
            )
    ...
    def _get_joint_pose(self) -> Goal:
        ...
        goal_joint_state = (
            self._goal if not self._relative else map(add, self._goal, self._start_joint_states)
        )
...
class Sequence(_AbstractCmd):
    ...
    def _get_sequence_request(self, move_group: MoveGroupCommander) -> [MotionSequenceItem]:
        items = []
...
class MoveGroupSequence:
    ...
    def __init__(self, move_group: MoveGroupCommander) -> None:
        ...
        self._service = rospy.ServiceProxy("plan_sequence_path", GetMotionSequence)
    ...
    def plan(self, command: _AbstractCmd) -> Tuple[bool, List[RobotTrajectory], float, int]:
        ...
        if isinstance(command, Sequence):
            ...
            items = [MotionSequenceItem(req=command._cmd_to_request(self._move_group))]
        else:
    ...
    def execute(self, trajectories: Optional[List[RobotTrajectory]] = None) -> None:
        ...
        for trajectory in to_execute:
            ...
            if not self._reached_pose(trajectory.joint_trajectory.points[-1].positions):
                raise MoveSequenceFailed("The robot did not reach it's target pose")
